...
--start-pid 品番(例:JMD-112)
    ...
    大文字小文字は区別しない。
    DMM作品ページのURL(cid=)から品番を作成しているが、URLのcidが
...
--start-cid DMM上の品番(例:15jmd112so)
    ...
    実際の品番と異なることがあり、このとき抜けやズレなどが発生する
    場合がある。
...
-e, --last-pid 品番
    ...
    大文字小文字は区別しない。
    DMM作品ページのURL(cid=)から品番を取得しているが、URLのcidが
    実際の品番と異なることがあり、このとき抜けやズレが発生する場合がある。
...
--last-cid DMM上の品番(例:15jmd112so)
    ...
    DMM上の品番とは、DMMサイト上でDMMが割り振った品番のこと。
    大文字小文字は区別しない。
    DMM作品ページのURL(cid=)から品番を取得しているが、URLのidが
    実際の品番と異なることがあり、このとき抜けやズレが発生する場合がある。
...
--pages-last ページ数
    DMM上で一覧ページを、最新順にこのページ数分だけデータを取得し
    作成する。1ページあたり120件。
...
-s, --series-link [シリーズ一覧ページ名]
    作成する女優ページ用ウィキテキストに同一のシリーズ一覧ページへの
    リンクを強制的に追加・変更する。
    既存のDMMの作品ページ上のものがあっても置き換える。

-l, --label-link [レーベル一覧ページ名]
    作成する女優ページ用ウィキテキストに同一のレーベル一覧ページへの
    リンクを強制的に追加・変更する。
    既存のDMMの作品ページ上のものがあっても置き換える。
